$.extend({
    ...
    JMHV: {
        CSSLink: function(link) {
            return JMHV.Path.CSSPath + link + '?v=' + JMHV.Path.Version
        },
        ScriptLink: function(link) {
            return JMHV.Path.ScriptPath + link + '?v=' + JMHV.Path.Version
        },
        ImageUrlLink: function(link) {
            return JMHV.Path.ImagePath + link + '?v=' + JMHV.Path.Version
        },
        ...
        format: function(template) {
            return $.JMHV.replace(template, [].slice.call(arguments, 1))
        },
        ...
        countDown: function(param) {
            ...
            param.data = r;
            param.dom.html($.JMHV.replace(param.temp, param.data));
            if (Math.floor((e.e - e.g) / 1000 <= 0) && param.callback) {
                param.callback();
                return
            }
            $.JMHV.countDown.Timer = setTimeout(function() {
                $.JMHV.countDown(param)
            }, param.time || 1000)
        },
        tmpl: function(data) {
            if (data.v.indexOf("url:") == 0) {
                ...
                data.v = $.JMHV.ScriptLink("template/" + link);
                var cache = $.JMHV.tmpl.getCache(data.v);
                if (cache) {
                    data.v = cache;
                    $.JMHV.tmpl.render(data)
                } else {
                    $.JMHV.tmpl.loadTmpl(data)
                }
            } else {
                $.JMHV.tmpl.render(data)
            }
        ...
        .method("loadTmpl", function(data) {
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState == 4) {
                    $.JMHV.tmpl.setCache(data.v, xhr.responseText);
                    data.v = xhr.responseText;
                    $.JMHV.tmpl.render(data)
                }
            }
        }).method("getCache", function(key) {
            $.JMHV.tmpl.cache = $.JMHV.tmpl.cache || {};
            return $.JMHV.tmpl.cache[key]
        }).method("setCache", function(key, value) {
            $.JMHV.tmpl.cache = $.JMHV.tmpl.cache || {};
            $.JMHV.tmpl.cache[key] = value
        ...
        if (child && tabtype) {
            (function(t, c, type, tcls, cls, time) {
                ...
                $(t).delegate(c, type, function(e) {
                    var callback = $(this).attr('JMHV-showcallback')
                      , id = $(this).attr('JMHV-showid');
                    if (mouseid == id) {
                        return false
                    }
                    mouseid = id;
